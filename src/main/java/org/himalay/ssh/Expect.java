...
/**
 * Krishna's Note
 * This file has been entirely derived from 
 * https://github.com/ronniedong/Expect-for-Java
 * and released and the same EPL license as the original one hae been. 
 * The reason I am including this as part of this project is so that I can provide it for maven builds and also these modifications
 * 1. Removed any static variable to make it thread safe.
 * 2. Added the prompt detection by waiting for timeout
 * 3. Added few additional convenience methods.
 */
...
public class Expect {
	...
	public Expect(InputStream input, OutputStream output) {
		try {
			...
			log.warn("Fatal error when initializing pipe or selector", e);
			//e.printStackTrace();
		}
		this.output = output;
	}
...
//						}
	...
	/**
	 * @param command The command to send 
	 * Convenience method to send a string to output handle, followed by a new line and then wait for prompt
	 */
	 ...
	 * 
	 * @param timeout Timeout in seconds
	 * @param patterns The patterns to wait for
...
//	}
	...
	public void setPrompt(String prompt)
	{
		...
		log.debug("Setting prompt to->|"+ prompt+"|<-");
		this.prompt = Pattern.compile(Pattern.quote((String) this.promptString));
	}
	...
	/**
	 * This will attempt to detect and set prompt
	 * @return The prompt as String
	 */
	public String detectPrompt()
	{
		String retVal = null;
		ArrayList<Pattern> patt = new ArrayList<Pattern>();
		patt.add(Pattern.compile("[.]*\n"));
		while ( RETV_TIMEOUT != expect(5,patt))
		{
			retVal = this.before+this.match;
		}
		retVal = this.buffer.toString();
		setPrompt(retVal);
		expectPrompt();
		return retVal;
	}
